...
function tryEnterNativeFullscreen(){
  ...
  try{
    ...
    if(el.requestFullscreen){const p=el.requestFullscreen({navigationUI:'hide'});if(p&&p.catch)p.catch(()=>{})}
  }catch(e){}
}
...
function stopSmoothMotion(){
  ...
  smoothElevator.raf=0;smoothElevator.moving=false;smoothElevator.queued=null;
}
function setMode(dest,{native=false}={}){
  const immersive=IMMERSIVE.has(dest),prev=document.body.dataset.playRoute||'';
  ...
  if(immersive&&native)tryEnterNativeFullscreen();else if(MENU.has(dest))tryExitNativeFullscreen();
}
...
function backgroundY(floorFloat){
  const ratio=Math.max(0,Math.min(1,(floorFloat-1)/19));
  return BG_TOP-(BG_TOP-BG_BOTTOM)*ratio;
}
function syncElevatorDom(floorFloat,final=false){
  ...
  if(photo){
    ...
    photo.style.backgroundPosition=`center ${backgroundY(floorFloat)}%`;
  }
...
function finishSmoothElevator(){
  ...
  const queued=smoothElevator.queued;smoothElevator.queued=null;
...
function upgradeElevator(){
  const track=document.querySelector('#floorTrack');
  if(!track)return;
  if(track.dataset.v5!=='1'){track.dataset.v5='1';track.insertAdjacentHTML('afterbegin',scenicMarkup())}
  const cabin=document.querySelector('.glass-cabin');if(cabin)cabin.classList.add('premium-panorama');
...
function detectCurrentScreen(){
  ...
  if(main.querySelector('.elevator-shell')){upgradeElevator();if(document.body.dataset.playRoute!=='elevator')setMode('elevator');return}
  if(main.querySelector('.bathroom-scene')&&document.body.dataset.playRoute!=='potty')setMode('potty');
}

document.addEventListener('click',e=>{
  const floorBtn=e.target.closest('.floor-key');
  if(floorBtn&&document.querySelector('.elevator-shell')){e.preventDefault();e.stopImmediatePropagation();startSmoothElevator(+floorBtn.dataset.floor);return}
},true);

document.addEventListener('click',e=>{
  const target=e.target.closest('[data-go]');if(!target)return;
  const dest=target.dataset.go;if(IMMERSIVE.has(dest)||MENU.has(dest))setMode(dest,{native:IMMERSIVE.has(dest)});
},true);
...
if(main){let scheduled=false;new MutationObserver(()=>{if(scheduled)return;scheduled=true;requestAnimationFrame(()=>{scheduled=false;detectCurrentScreen()})}).observe(main,{childList:true})}
