...
(() => {
  ...
  function nativeStrictReady() {
    ...
    try {
      if (typeof bridge.strictReady === "function")
        return bridge.strictReady() === true;
      if (
        typeof bridge.isProfileOwner === "function" &&
        bridge.isProfileOwner() === true
      )
        return true;
      if (typeof bridge.isDeviceOwner === "function")
        return bridge.isDeviceOwner() === true;
    } catch {}
    return false;
  }
  function nativePolicyRole() {
    const bridge = nativeBridge();
    if (!bridge) return "web-only";
    try {
      if (typeof bridge.policyRole === "function")
        return bridge.policyRole() || "none";
    } catch {}
    return "none";
  }
  function syncNativeKiosk() {
    ...
    try {
      if (locked) {
        if (nativeStrictReady() !== true) return false;
        bridge.lock();
      } else bridge.unlock();
      return true;
    } catch {
  ...
  async function digestPin(pin, salt) {
    ...
    const digest = await crypto.subtle.digest(
      "SHA-256",
      encoder.encode(`seowoo-parent-pin-v2:${salt}:${pin}`),
    );
  ...
  function ensureDialog() {
    let dlg = document.querySelector("#screenLockDialog");
    if (dlg) return dlg;
    dlg = document.createElement("dialog");
    dlg.id = "screenLockDialog";
    dlg.className = "screen-lock-dialog";
    ...
    dlg
      .querySelector(".screen-lock-cancel")
  ...
  function syncButtons() {
    for (const btn of buttons()) {
      ...
      btn.setAttribute(
        "aria-label",
        locked
          ? "화면 잠금 켜짐. 눌러서 부모 비밀번호 입력 후 해제"
          : "화면 잠금 꺼짐. 눌러서 부모 비밀번호 입력 후 잠금",
      );
  ...
  function applyState(announce = false) {
    ...
    const ready = nativeStrictReady();
    document.documentElement.dataset.nativeKiosk =
      ready === true
        ? "strict"
        : ready === false
          ? "setup-required"
    ...
    window.dispatchEvent(
      new CustomEvent("seowoo:screenlock", {
        detail: {
          locked,
          nativeStrictReady: ready,
          policyRole: nativePolicyRole(),
        },
      }),
    );
    if (announce) toast(locked ? "화면잠금을 켰어요" : "화면잠금을 껐어요");
  }
  function setLocked(next, announce = true) {
    ...
    applyState(announce);
    try {
      navigator.vibrate?.(locked ? [45, 55, 80] : [80, 45, 45]);
    } catch {}
  }
  ...
  function showError(msg) {
    const dlg = ensureDialog(),
      el = dlg.querySelector("#screenLockError");
    el.textContent = msg;
    dlg.querySelector("#screenLockPin")?.focus();
  }
  function closeDialog() {
    const dlg = document.querySelector("#screenLockDialog");
    if (!dlg) return;
    try {
      dlg.close();
    } catch {}
    dlg.querySelector("form")?.reset();
    const err = dlg.querySelector("#screenLockError");
    if (err) err.textContent = "";
  }
  function strictLockBlocked() {
    return nativeBridge() && nativeStrictReady() !== true;
  }
  function openPinDialog() {
    const dlg = ensureDialog(),
      setup = !readCredential(),
      title = dlg.querySelector("#screenLockDialogTitle"),
      text = dlg.querySelector("#screenLockDialogText"),
      confirmWrap = dlg.querySelector("#screenLockConfirmWrap"),
      submit = dlg.querySelector(".screen-lock-submit");
    dlg.dataset.mode = setup ? "setup" : "verify";
    if (setup) {
      title.textContent = "부모 비밀번호 설정";
      text.textContent = locked
        ? "새 4~8자리 숫자 비밀번호를 설정하면 기존 잠금을 해제할 수 있어요."
        : "화면잠금에 사용할 4~8자리 숫자 비밀번호를 정해줘.";
      confirmWrap.hidden = true;
      submit.textContent = locked ? "설정 후 잠금 해제" : "설정 후 잠금 켜기";
    } else {
      title.textContent = locked ? "화면잠금 해제" : "화면잠금 켜기";
      text.textContent = locked
        ? "부모 비밀번호를 입력하면 잠금이 풀리고 부모 사용자로 돌아갈 수 있는 화면이 열려요."
        : strictLockBlocked()
          ? "Y700에 서우 전용 사용자 설정이 아직 필요해. 메인 구글계정은 건드리지 않는 보조 사용자 방식으로 설정해야 해."
          : nativeBridge()
            ? "부모 비밀번호로 전용 모드를 켭니다."
            : "앱 안에서 외부 링크와 뒤로가기를 제한합니다. 기기의 홈 화면까지 잠그려면 iPhone 사용법 유도 또는 Android 전용 모드를 사용해 주세요.";
      confirmWrap.hidden = true;
      submit.textContent = locked ? "잠금 해제" : "잠금 켜기";
    }
    dlg.querySelector("#screenLockError").textContent = "";
    dlg.querySelector("form").reset();
    if (!dlg.open) dlg.showModal();
    setTimeout(() => dlg.querySelector("#screenLockPin")?.focus(), 20);
  }
  async function handlePinSubmit(e) {
    e.preventDefault();
    const dlg = ensureDialog(),
      pin = dlg.querySelector("#screenLockPin").value.trim(),
      mode = dlg.dataset.mode;
    if (Date.now() < blockedUntil) {
      showError(
        `비밀번호를 여러 번 틀렸어. ${Math.ceil((blockedUntil - Date.now()) / 1000)}초 후 다시 시도해줘.`,
      );
      return;
    }
    if (!PIN_RE.test(pin)) {
      showError("4~8자리 숫자로 입력해줘.");
      return;
    }
    const submit = dlg.querySelector(".screen-lock-submit");
    submit.disabled = true;
    try {
      if (mode === "setup") {
        if (!locked && strictLockBlocked()) {
          showError(
            "Y700 서우 전용 사용자 설정이 아직 안 됐어. PC에서 보조 사용자를 만든 뒤 이 앱을 그 사용자의 Profile Owner로 1회 등록해줘. 메인 구글계정/게임 데이터는 건드리지 않아.",
          );
          return;
        }
        ...
        const next = !locked;
        setLocked(next, true);
        if (next && !nativeBridge()) requestFullscreenContainment();
        else if (!next) releaseFullscreenContainment();
        return;
      }
      ...
      if (!ok) {
        ...
        if (failedAttempts >= 5) {
          ...
          failedAttempts = 0;
          showError("비밀번호를 5번 틀렸어. 30초 후 다시 시도해줘.");
        } else showError(`비밀번호가 맞지 않아. (${failedAttempts}/5)`);
        return;
      }
      if (!locked && strictLockBlocked()) {
        showError(
          "Y700 서우 전용 사용자 설정이 아직 안 됐어. 메인 사용자를 초기화하지 말고, 보조 사용자 + Profile Owner 설정을 먼저 진행해줘.",
        );
        return;
      }
      failedAttempts = 0;
      ...
      showError("비밀번호 처리 중 오류가 났어. 앱을 다시 열고 시도해줘.");
    } finally {
  ...
  function installGlobalGuards() {
    ...
    document.addEventListener(
      ...
      (e) => {
        if (
          locked &&
          !e.target.closest?.(".screen-lock-toggle,.screen-lock-dialog")
        )
  ...
  function init() {
    const headerBtn = document.querySelector("#screenLockBtn"),
      floatBtn = ensureFloating();
    ensureDialog();
    bindButton(headerBtn);
    bindButton(floatBtn);
    installGlobalGuards();
    locked = readLocked();
    if (locked && nativeBridge() && nativeStrictReady() !== true) {
      locked = false;
      saveLocked();
      toast("Y700 서우 전용 사용자 설정이 필요해서 기존 임시 잠금을 해제했어.");
    }
    applyState(false);
  }
  ...
  window.SeowooScreenLock = {
    ...
    saveCredential,
    setFromParent(next) {
      if (next && strictLockBlocked()) {
        toast("전용 모드 설정이 필요해요");
        return false;
      }
      setLocked(next);
      return true;
    ...
    get strictReady() {
      return nativeStrictReady();
    },
    get policyRole() {
      return nativePolicyRole();
    },
  };
})();
