...
(() => {
  ...
  let initial = 1;
  try {
    initial = clamp(sessionStorage.getItem("seowoo-floor"));
  } catch {}
  const state = {
    current: initial,
    position: initial,
  ...
  const save = () => {
    try {
      sessionStorage.setItem("seowoo-floor", state.current);
    } catch {}
  };
  const setText = (el, t) => {
  ...
  function doorFraction(now = performance.now()) {
    if (state.phase === "closing" || state.phase === "opening") {
      const t =
        state.doorDuration <= 0
          ? 1
          : clamp01((now - state.at) / state.doorDuration);
    ...
    if (
      state.phase === "idle" ||
      state.phase === "waiting" ||
      state.phase === "preclose"
    )
      return state.door;
    return state.door;
  }
  function animateDoor(to, now, mode = "manual") {
    const from = doorFraction(now);
    state.door = from;
    state.doorFrom = from;
    state.doorTo = to;
  ...
  function begin(target) {
    ...
    if (doorFraction() > 0.985) {
      ...
      state.doorTo = 1;
      state.doorDuration = 0;
      state.closeMode = "trip";
      phase("closing", performance.now());
      return;
    }
    state.door = 0;
    phase("preclose", performance.now());
  }
  function doorLabel() {
    if (state.phase === "travel")
      return state.target > state.start ? "올라가요" : "내려가요";
  ...
  function updateControls() {
    ...
    if (dom.close) {
      ...
      dom.close.setAttribute("aria-disabled", String(closeDisabled));
    }
  }
  function paint() {
    if (!dom) return;
    ...
    if (signature !== lastPaint) {
      ...
      dom.keys.forEach((key) => {
        ...
        key.setAttribute(
          "aria-pressed",
          String(selected || n === state.queued),
        );
        if (n === state.current) key.setAttribute("aria-current", "true");
        else key.removeAttribute("aria-current");
      });
      updateControls();
    }
    let closed = doorFraction();
    if (matchMedia("(prefers-reduced-motion: reduce)").matches)
      closed = ["closing", "travel", "arrival", "closed"].includes(state.phase)
        ? 1
        : 0;
    state.door = closed;
    dom.shell.style.setProperty("--door-closed", closed.toFixed(3));
    dom.left.style.transform = `translate3d(${-101 * (1 - closed)}%,0,0)`;
    dom.right.style.transform = `translate3d(${101 * (1 - closed)}%,0,0)`;
  ...
  function requestOpen(quiet = false) {
    if (!dom || state.phase === "travel") return false;
    if (!quiet) audio()?.button();
    ...
    if (state.phase === "preclose" || state.phase === "closing") {
      if (
        (state.phase === "preclose" || state.closeMode === "trip") &&
        state.target !== state.current
      ) {
        state.queued = state.target;
        state.target = state.current;
        state.start = state.current;
      }
      audio()?.doorOpen();
  ...
  function requestClose() {
    if (!dom || state.openHeld) return false;
    audio()?.button();
    const now = performance.now();
    if (
      state.phase === "travel" ||
  ...
  function distance(ms) {
    ...
    if (ms > state.duration - RAMP_MS)
      return total - (state.duration - ms) ** 2 / (2 * RAMP_MS * FLOOR_MS);
    return (ms - RAMP_MS / 2) / FLOOR_MS;
  }
  function tick(now) {
    state.raf = 0;
    if (!dom?.shell.isConnected) {
      unmount();
      return;
    }
    if (document.hidden || state.paused !== null) return;
    const elapsed = now - state.at;
    if (state.phase === "preclose" && elapsed >= PRE_CLOSE_MS) {
      audio()?.doorClose();
      state.door = 0;
      animateDoor(1, now, "trip");
    } else if (
      state.phase === "closing" &&
      elapsed >=
        state.doorDuration + (state.closeMode === "trip" ? POST_CLOSE_MS : 0)
    ) {
      state.door = 1;
      ...
      const moved = distance(Math.min(elapsed, state.duration)),
        direction = Math.sign(state.target - state.start);
      state.position = state.start + direction * moved;
      state.current = clamp(state.position);
      ...
      state.phase === "waiting" &&
      !state.openHeld &&
      elapsed >= DOOR_HOLD_MS
    ) {
      const next = state.queued;
      state.queued = null;
      phase("idle", now);
      if (next !== null && next !== state.current) begin(next);
    }
    paint();
    schedule();
  }
  function ensureDoorControls(shell) {
    ...
    if (!panel) return { open: null, close: null };
    let controls = panel.querySelector(".door-controls");
    if (!controls) {
      controls = document.createElement("div");
      controls.className = "door-controls";
      controls.setAttribute("role", "group");
      controls.setAttribute("aria-label", "엘리베이터 문 조작");
      controls.innerHTML =
        '<button type="button" class="door-control door-open" data-door-action="open" aria-label="엘리베이터 문 열기" aria-pressed="false"><span class="door-symbol" aria-hidden="true">◁&nbsp;&nbsp;▷</span><small>열림</small></button><button type="button" class="door-control door-close" data-door-action="close" aria-label="엘리베이터 문 닫기"><span class="door-symbol" aria-hidden="true">▷&nbsp;&nbsp;◁</span><small>닫힘</small></button>';
      panel.append(controls);
    }
    return {
      open: controls.querySelector('[data-door-action="open"]'),
      close: controls.querySelector('[data-door-action="close"]'),
    };
  }
  function suppressNativeGesture(e) {
    if (dom?.shell?.contains(e.target)) e.preventDefault();
  }
  function openHoldStart(e) {
    ...
    state.openHeld = true;
    try {
      dom.open.setPointerCapture(e.pointerId);
    } catch {}
    requestOpen();
    paint();
  }
  function openHoldEnd(e) {
    ...
    try {
      if (e && dom.open.hasPointerCapture?.(e.pointerId))
        dom.open.releasePointerCapture(e.pointerId);
    } catch {}
    if (state.phase === "waiting") phase("waiting", performance.now());
    else paint();
  }
  function unmount() {
  ...
  function onClick(e) {
    ...
    if (door && dom?.shell.contains(door)) {
      if (door.dataset.doorAction === "open") {
        if (e.detail === 0) requestOpen();
        return;
      }
      requestClose();
      return;
    }
    const key = e.target.closest("[data-floor]");
    if (key && dom?.shell.contains(key)) select(key.dataset.floor);
  }
  function mount(shell) {
    if (!shell || dom?.shell === shell) return;
    unmount();
    const controls = ensureDoorControls(shell);
    dom = {
      shell,
      floor: shell.querySelector("#elevatorFloor"),
      arrow: shell.querySelector("#elevatorArrow"),
      message: shell.querySelector("#elevatorMsg"),
      left: shell.querySelector(".door-l"),
      right: shell.querySelector(".door-r"),
      keys: [...shell.querySelectorAll("[data-floor]")],
      open: controls.open,
    ...
    panorama = window.SeowooPanorama.mount(
      shell.querySelector(".outside-view"),
    );
  ...
  function pause() {
    save();
    if (!dom || state.paused !== null) return;
    state.openHeld = false;
    if (dom.open) {
      dom.open.classList.remove("held");
      dom.open.setAttribute("aria-pressed", "false");
    }
    state.paused = performance.now();
  ...
  function resume() {
    ...
    if (state.paused !== null) {
      state.at += performance.now() - state.paused;
      state.paused = null;
      if (state.phase === "travel") audio()?.motorStart();
      if (state.phase === "waiting") state.at = performance.now();
    }
    panorama?.resize();
    paint();
    schedule();
  ...
  function setMode(route) {
    if (route !== "elevator") {
      unmount();
      if (!IMMERSIVE.has(route) && document.fullscreenElement)
        document.exitFullscreen().catch(() => {});
    }
    document.body.classList.toggle("game-fullscreen", IMMERSIVE.has(route));
