...
server.stdout.once('data',async()=>{try{await run('tests/audio.cjs');await run('tests/polish.cjs');await run('tests/browser.cjs');await run('tests/screen-lock.cjs');await run('tests/pwa-update.cjs')}catch(e){console.error(e);process.exitCode=1}finally{server.kill()}});
